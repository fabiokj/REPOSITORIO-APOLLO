...
url_b64_principal = 'aHR0cDovL2RldXNhcG9sbG8uY29tL0FQT0xMTy9CYXNlLVByaW5jaXBhbC9CYXNlLVByaW5jaXBhbC5odG1s'
url_principal = base64.b64decode(url_b64_principal)
#######################################################################################
####  MENSAGEM DO BEM VINDOS BASE 64 ######################################################
titulo_boas_vindas = 'BEM-VINDOS'
url_b64_bem_vindo = 'aHR0cHM6Ly9naXRsYWIuY29tL3NpbHZhZGVpdmlzb241NjYvZ2l0cGxheS9yYXcvbWFzdGVyL0JPQVMlMjBWSU5EQVM='
url_mensagem_bem_vindo = base64.b64decode(url_b64_bem_vindo)
#######################################################################################
####  MENSAGEM DO STATUS BASE 64 ##############################################################
url_b64_mensagem = 'aHR0cHM6Ly9naXRsYWIuY29tL3NpbHZhZGVpdmlzb241NjYvZ2l0cGxheS9yYXcvbWFzdGVyL1NUQVRVUyUyMEFERC1PTg=='
...
#### MENU VIP ################################################################
titulo_vip = '[COLOR deepskyblue][B]CLIQUE PARA CONHECER O[/COLOR][/B][COLOR gold][B] VIP [/COLOR][/B]'
thumb_vip = 'https://i.imgur.com/yex4wzR.png'
#### DESCRIÇÃO ADVANCED VIP ###########################################################
url_b64_vip_descricao = 'aHR0cHM6Ly9naXRsYWIuY29tL3NpbHZhZGVpdmlzb241NjYvZ2l0cGxheS8tL3Jhdy9NZXN0cmUvREVTQ1JJJUMzJTg3JUMzJTgzTyUyMEFEVkFOQ0VEJTIwVklQ'
url_desc_vip = base64.b64decode(url_b64_vip_descricao)
#### DIALOGO ADVANCED VIP - SERVIDOR DESATIVADO - CLICK ###################################
url_b64_vip_dialogo = 'aHR0cHM6Ly9naXRsYWIuY29tL3NpbHZhZGVpdmlzb241NjYvZ2l0cGxheS8tL3Jhdy9NZXN0cmUvRElBTE9HTyUyMEFEVkFOQ0VEJTIwVklQ'
url_vip_dialogo = base64.b64decode(url_b64_vip_dialogo)
...
useragent ='\x39\x2e\x33\x2e\x35\x2f\x61\x67\x74\x73\x6a\x74'
##############################################################################
### DIALOGO
...
## MENU ATUALIZAÇÃO
menu_atualizacao = '[COLOR orange][B]->[/COLOR][/B][B][COLOR deepskyblue] ATUALIZAÇÃO[/COLOR][/B]'
thumb_update = 'https://i.imgur.com/rmxMKWj.jpg'
url_b64_atualizacao = 'aHR0cHM6Ly9naXRsYWIuY29tL3NpbHZhZGVpdmlzb241NjYvZ2l0cGxheS8tL3Jhdy9NZXN0cmUvQ29uZmlnLnB5'
...
#Modulos
exec(module_import('https://gitlab.com/silvadeivison566/gitplay/-/raw/master/modulos.py'))
                ...
                if len(url) > 1:
                    ...
                    if isXMLSource:
                            if not regexs == None: #<externallink> and <regex>
                                ...
                                #addLink(url[0],name.encode('utf-8', 'ignore')+  '[COLOR deepskyblue]build XML[/COLOR]',thumbnail,fanArt,desc,genre,date,True,None,regexs,total)
                            else:
...
def urlsolver(url):
    ...
    ##genesis_url = 'https://raw.githubusercontent.com/lambda81/lambda-addons/master/plugin.video.genesis/commonresolvers.py'
        genesis_url = 'https://raw.githubusercontent.com/APOLLOHD/OneRepo/master/plugin.video.genesis/commonresolvers.py'
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from APOLLO Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if showcontext == '!!update':
                ...
                contextMenu.append(('[COLOR deepskyblue]!!update[/COLOR]','XBMC.RunPlugin(%s)' %fav_params2))
            if not name in FAV:
                fav_params = (
                ...
                ##contextMenu.append(('Adicionar [COLOR deepskyblue]APOLLO[/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            dialog = xbmcgui.Dialog()
            ret = dialog.yesno('            Movie Title', 'PLAY :: %s ?'%name)
            if ret:
...
#                print 'setting as folder in link'
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false" and '$$LSPlayOnlyOne$$' not in playlist[0] :
                ...
                name = name + '[COLOR deepskyblue] (' + str(len(playlist)) + ' itens)[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
